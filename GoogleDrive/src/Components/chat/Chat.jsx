...
//   return (
//     <>
//       <button onClick={() => setIsAppOpen(!isAppOpen)} className="toggle-button">
//         {isAppOpen ? <img className="close" src={Close} alt="" /> : <img className="chht" src={Chats} alt="" />}
//       </button>
...
//                   <div key={index} className={`chat-item ${chat.type === "question" ? "chat-question" : "chat-answer"}`}>
...
//                 }}
//               ></textarea>
//               <button type="submit" className={`input-button ${generatingAnswer ? "disabled" : ""}`} disabled={generatingAnswer}>
//                 Send
//               </button>
//             </form>
//           </div>
...
const Chat = () => {
  ...
  async function generateAnswer(e) {
    ...
    try {
      ...
      const response = await axios({
        url: `https://generativelanguage.googleapis.com/v1beta/models/gemini-pro:generateContent?key=${
          import.meta.env.VITE_API_GENERATIVE_LANGUAGE_CLIENT // Using the API key
        }`,
        method: "post",
        data: {
          contents: [{ parts: [{ text: question }] }], // Request body format for the Gemini API
        },
      });
